...
# FE 1 Cyberpunk Mission Control HUD (Default Experience)
@app.get("/", response_class=HTMLResponse)
@app.get("/hud", response_class=HTMLResponse)
def get_mission_control_hud():
    hud_path = Path(__file__).resolve().parent.parent.parent / "frontend" / "aegis_mission_control.html"
    if hud_path.exists():
        with open(hud_path, "r", encoding="utf-8") as f:
            return f.read()
    return HTMLResponse("<h1>HUD HTML not found</h1>", status_code=404)
...
# Cyberpunk Ambient Operator Diagnostic Console
...
<body class="p-6">
    <div class="max-w-6xl mx-auto space-y-6">
        ...
        <div class="flex items-center justify-between border-b border-slate-800 pb-4">
            ...
            <div class="flex items-center space-x-3">
                <a href="/hud" class="px-3 py-1 text-xs rounded border border-purple-500 bg-purple-950/60 text-purple-300 hover:bg-purple-900/80 font-bold transition">✨ Open FE 1 Full HUD (/hud)</a>
